...
function get_account_log($user_id, $num = 15, $start, $process_type = '')
{
    ...
    $res = $db->orderBy('add_time', 'desc')->take($num)->skip($start->start_id - 1)->get();
    ...
    if (!empty($res)) {
        ...
        foreach ($res as $key => $rows) {
            ...
            $rows['short_admin_note'] = ($rows['admin_note'] > '') ? RC_String::sub_str($rows['admin_note'], 30) : __('暂无', 'finance');
            $rows['user_note']        = nl2br(htmlspecialchars($rows['user_note']));
            $rows['short_user_note']  = ($rows['user_note'] > '') ? RC_String::sub_str($rows['user_note'], 30) : __('暂无', 'finance');
            $rows['pay_status']       = ($rows['is_paid'] == 0) ? __('未确认', 'finance') : __('已完成', 'finance');
            $rows['format_amount']    = price_format(abs($rows['amount']), false);

            /* 会员的操作类型： 冲值，提现 */
            if ($rows['process_type'] == 0) {
                $rows['type'] = __('充值', 'finance');
            } else {
                $rows['type'] = __('提现', 'finance');
            }
